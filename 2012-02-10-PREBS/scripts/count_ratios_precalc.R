...
mapped_to_gene_regions <- sum(!is.na(findOverlaps(aligns, exonRanges, select = "first", ignore.strand = TRUE)))
...
mapped_to_probe_regions <- sum(!is.na(findOverlaps(aligns, my_ranges, select = "first", ignore.strand = TRUE)))
