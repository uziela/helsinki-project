...
<gap> - gap between paired-end reads (only used in paired-end read run)
...
if [ ! -d $input_dir/tophat ] ; then
	echo "------------------- stage 2 -------------------"
	echo "------------------- stage 2 -------------------" 1>&2
	mkdir $input_dir/tophat
	if [ $fasta != 0 ] ; then
		echo "Aligning fasta unpaired reads"
		tophat -p 8 --transcriptome-only --GTF ./Human_transcriptome/Homo_sapiens.GRCh37.65.gtf --transcriptome-index=./Human_transcriptome/known --output-dir $input_dir/tophat hg19 $fasta_files
	elif [ $paired == 0 ] ; then
		echo "Aligning fastq unpaired reads"
		tophat -p 8 --transcriptome-only --GTF ./Human_transcriptome/Homo_sapiens.GRCh37.65.gtf --transcriptome-index=./Human_transcriptome/known --output-dir $input_dir/tophat hg19 $fastq_files
	elif [ $paired == 1 ] ; then
		echo "Aligning fastq paired reads"
		tophat -p 8 -r $gap --transcriptome-only --GTF ./Human_transcriptome/Homo_sapiens.GRCh37.65.gtf --transcriptome-index=./Human_transcriptome/known --output-dir $input_dir/tophat hg19 $fastq_files1 $fastq_files2
	elif [ $paired == 2 ] ; then
		fastq1a=`echo $fastq_files1 | cut -d " " -f 1`
		fastq1b=`echo $fastq_files1 | cut -d " " -f 2`
		fastq2a=`echo $fastq_files2 | cut -d " " -f 1`
		fastq2b=`echo $fastq_files2 | cut -d " " -f 2`
		tophat -p 8 -r $gap --transcriptome-only --GTF ./Human_transcriptome/Homo_sapiens.GRCh37.65.gtf --transcriptome-index=./Human_transcriptome/known --output-dir $input_dir/tophat_a hg19 $fastq1a $fastq2a
		tophat -p 8 -r $gap --transcriptome-only --GTF ./Human_transcriptome/Homo_sapiens.GRCh37.65.gtf --transcriptome-index=./Human_transcriptome/known --output-dir $input_dir/tophat_b hg19 $fastq1b $fastq2b
		samtools sort -n $input_dir/tophat_a/accepted_hits.bam $input_dir/tophat_a/accepted_hits_sorted
		samtools sort -n $input_dir/tophat_b/accepted_hits.bam $input_dir/tophat_b/accepted_hits_sorted
		mkdir $input_dir/tophat
		samtools merge -n $input_dir/tophat/accepted_hits.bam $input_dir/tophat_a/accepted_hits_sorted.bam $input_dir/tophat_b/accepted_hits_sorted.bam
	else 
		echo "ERROR: more then two pairs of paired ended reads is not supported"
	fi
fi

if [ ! -f $input_dir/accepted_hits.RData ] ; then
	echo "------------------- stage 3 -------------------"
	echo "------------------- stage 3 -------------------" 1>&2
	...
	Rscript scripts/bam_to_R.R $input_dir/accepted_hits_final.bam $input_dir/accepted_hits.RData
fi
...
if [ $mode == "RANGES" ] ; then
	...
	if [ ! -f $input_dir/accepted_hits_counts.RData ] ; then
		echo "------------------- stage 5 -------------------"
		echo "------------------- stage 5 -------------------" 1>&2
		Rscript scripts/getCounts.R $input_dir/accepted_hits
	fi
fi
...
if [ ! -d $input_dir/plots ] ; then
	...
	Rscript scripts/array_comp.R $array_expr_means $input_dir $input_dir/$run_name"_PREBS.RData" $mmseq_file $input_dir/accepted_hits_counts.RData $run_name
