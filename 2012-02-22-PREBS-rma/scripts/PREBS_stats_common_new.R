...
my_filter <- function(merged_filtered, mode, percentage_upper, percentage_lower) {
	...
	if (mode == "ARRAY_BEST") {
		merged_filtered <- merged_filtered[is.finite(merged_filtered$mmseq_expr),]
		merged_filtered <- merged_filtered[is.finite(merged_filtered$RPKM),]
		merged_filtered <- merged_filtered[is.finite(merged_filtered$array_expr),]
		merged_filtered <- merged_filtered[is.finite(merged_filtered$PREBS_RMA),]
		merged_filtered <- merged_filtered[order(merged_filtered$array_expr, decreasing=TRUE),]
	}

	if (mode == "ARRAY_MEAN") {
		merged_filtered <- merged_filtered[is.finite(merged_filtered$array_log2fc),]
		...
		merged_filtered <- merged_filtered[order(merged_filtered$array_expr1+merged_filtered$array_expr2, decreasing=TRUE),]
	}
...
for (i in 1:length(merged_files)) {
	...
	merged_filtered <- my_filter(merged_with_rma, "ARRAY_BEST", PERCENT_UPPER, PERCENT_LOWER)
	...
	#my_plot(rank(merged_filtered$PREBS_RMA), rank(merged_filtered$array_expr), paste(output_dir,"/plots/",sample_name,"_PREBS_RMA_vs_array_rank.png", sep=""), "PREBS_RMA value", "Microarray expression")
	...
	#my_plot(rank(merged_filtered$mmseq_expr), rank(merged_filtered$array_expr), paste(output_dir,"/plots/",sample_name,"_mmseq_vs_array_rank.png", sep=""), "MMseq expression (log2 scale)", "Microarray expression")
	...
	#my_plot(rank(merged_filtered$RPKM), rank(merged_filtered$array_expr), paste(output_dir,"/plots/",sample_name,"_RPKM_vs_array_rank.png", sep=""), "RPKM (log2 scale)", "Microarray expression")	
...
for (i in 1:(N_SAMPLES-1)) {
	for (j in (i+1):N_SAMPLES) {
		...
		merged_filtered <- my_filter(merged, "ARRAY_MEAN", PERCENT_UPPER, PERCENT_LOWER)
		if (mode2 == "CROSS_DIFF") {
