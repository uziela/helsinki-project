...
my_filter <- function(merged_filtered, mode, percentage_upper, percentage_lower) {
	...
	if (mode == "PREBS_INTER") {
		merged_filtered <- merged_filtered[is.finite(merged_filtered$prebs_log2fc),]
		merged1 <- merged_filtered[order(merged_filtered$PREBS_RMA1, decreasing=TRUE),]
		merged2 <- merged_filtered[order(merged_filtered$PREBS_RMA2, decreasing=TRUE),]
		merged1 <- merged1[upper:lower,]
		merged2 <- merged2[upper:lower,]
		merged1 <- merged1[is.element(merged1$Gene_ID, merged2$Gene_ID),]
		merged_filtered <- merged1
	}
	
	if (mode == "RPKM_INTER") {
		merged_filtered <- merged_filtered[is.finite(merged_filtered$rpkm_log2fc),]
		merged1 <- merged_filtered[order(merged_filtered$RPKM1, decreasing=TRUE),]
		merged2 <- merged_filtered[order(merged_filtered$RPKM2, decreasing=TRUE),]
		merged1 <- merged1[upper:lower,]
		merged2 <- merged2[upper:lower,]
		merged1 <- merged1[is.element(merged1$Gene_ID, merged2$Gene_ID),]
		merged_filtered <- merged1
	...
	if (mode == "MMSEQ_INTER") {
		merged_filtered <- merged_filtered[is.finite(merged_filtered$mmseq_log2fc),]
		merged1 <- merged_filtered[order(merged_filtered$mmseq_expr1, decreasing=TRUE),]
		merged2 <- merged_filtered[order(merged_filtered$mmseq_expr2, decreasing=TRUE),]
		merged1 <- merged1[upper:lower,]
		merged2 <- merged2[upper:lower,]
		merged1 <- merged1[is.element(merged1$Gene_ID, merged2$Gene_ID),]
		merged_filtered <- merged1
...
for (i in 1:length(merged_files)) {
	...
	if (i == index1) { 
		write.table(cbind(merged_filtered$PREBS_RMA, merged_filtered$array_expr), file=paste(output_dir, "/plot-data/", sample_name,"_PREBS_RMA_vs_array.txt", sep=""), row.names=FALSE, col.names=FALSE)
	}
	...
	if (i == index1) { 
		write.table(cbind(merged_filtered$mmseq_expr, merged_filtered$array_expr), file=paste(output_dir, "/plot-data/", sample_name,"_mmseq_vs_array.txt", sep=""), row.names=FALSE, col.names=FALSE)
	}
	...
	if (i == index1) { 
		write.table(cbind(merged_filtered$RPKM, merged_filtered$array_expr), file=paste(output_dir, "/plot-data/", sample_name,"_RPKM_vs_array.txt", sep=""), row.names=FALSE, col.names=FALSE)
	}
...
for (i in 1:(N_SAMPLES-1)) {
    ...
    if (ncol(merged1) == 7) {
		...
		merged_filtered <- my_filter(merged, "MMSEQ_INTER", PERCENT_UPPER, PERCENT_LOWER)
		if (mode2 == "CROSS_DIFF") {
		...
		if ((i == index1) && (j == index2)) {
			write.table(cbind(merged_filtered$mmseq_log2fc, merged_filtered$array_log2fc), file=paste(output_dir, "/plot-data/mmseq_vs_array_log2fc.txt", sep=""), row.names=FALSE, col.names=FALSE)
		...
		merged_filtered <- my_filter(merged, "PREBS_INTER", PERCENT_UPPER, PERCENT_LOWER)
		if (mode2 == "CROSS_DIFF") {
			merged_filtered <- my_replace_ranks_new_diff(merged_filtered)
		...
		if ((i == index1) && (j == index2)) {
			write.table(cbind(merged_filtered$prebs_log2fc, merged_filtered$array_log2fc), file=paste(output_dir, "/plot-data/prebs_vs_array_log2fc.txt", sep=""), row.names=FALSE, col.names=FALSE)
		...
		merged_filtered <- my_filter(merged, "RPKM_INTER", PERCENT_UPPER, PERCENT_LOWER)
		if (mode2 == "CROSS_DIFF") {
			merged_filtered <- my_replace_ranks_new_diff(merged_filtered)
		...
		if ((i == index1) && (j == index2)) {
			write.table(cbind(merged_filtered$rpkm_log2fc, merged_filtered$array_log2fc), file=paste(output_dir, "/plot-data/rpkm_vs_array_log2fc.txt", sep=""), row.names=FALSE, col.names=FALSE)
