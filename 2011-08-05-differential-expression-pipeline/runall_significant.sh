...
if [ ! -f $input_dir/$run_name.table ] ; then
	...
	Rscript scripts/remove_insignificant.R $input_dir/$run_name.RData $input_dir/"$run_name"_signif.RData $input_dir/"$run_name"_signif.table		# create a separate table without very significant values
fi


if [ ! -d $input_dir/plots ] ; then
	mkdir $input_dir/plots
	Rscript scripts/plotGraph.R $input_dir/"$run_name"_signif.RData $input_dir/plots
fi

if [ ! -d $input_dir/histograms ] ; then
	mkdir $input_dir/histograms
	Rscript scripts/histograms.R $input_dir/"$run_name"_signif.RData $input_dir/histograms
fi

if [ ! -d $input_dir/more-plots ] ; then
	mkdir $input_dir/more-plots
	./more-plots.sh $input_dir/"$run_name"_signif.table $input_dir/more-plots
fi
